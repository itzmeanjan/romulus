...
// Compile it with
//
// g++ -Wall -std=c++20 -O3 -march=native -I include example/romulusm.cpp
int main() {
  ...
  romulusm::encrypt(key, nonce, data, dlen, txt, enc, mlen, tag);
  f0 = romulusm::decrypt(key, nonce, tag, data, dlen, enc, dec, mlen);
  ...
  std::cout << "Romulus-M AEAD" << std::endl << std::endl;
