...
// Compile it with
//
// g++ -Wall -std=c++20 -O3 -march=native -I include example/romulusn.cpp
int main() {
  ...
  romulusn::encrypt(key, nonce, data, dlen, txt, enc, mlen, tag);
  f0 = romulusn::decrypt(key, nonce, tag, data, dlen, enc, dec, mlen);
  ...
  std::cout << "Romulus-N AEAD" << std::endl << std::endl;
